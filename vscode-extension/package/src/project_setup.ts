...
export class ProjectSetupManager {
    private api: CodeGuardAPI;
    
    constructor(api: CodeGuardAPI) {
        this.api = api;
    }
    
    async showProjectTemplates(): Promise<void> {
        try {
    ...
    private async showProjectPreview(preview: ProjectPreview, projectPath: string): Promise<boolean> {
    ...
    private getPreviewHtml(preview: ProjectPreview, projectPath: string): string {
        return `
            <!DOCTYPE html>
            <html lang="en">
            <head>
    ...
    private async showSetupGuide(projectResult: any): Promise<void> {
        const panel = vscode.window.createWebviewPanel(
            'setupGuide',
            'Setup Guide',
            vscode.ViewColumn.One,
    ...
    private getSetupGuideHtml(project: any): string {
        return `
            <!DOCTYPE html>
            <html lang="en">
            <head>
            ...
            <body>
                ...
                ${project.setup_commands ? project.setup_commands.map((cmd: string, i: number) => `
                <div class="step">
